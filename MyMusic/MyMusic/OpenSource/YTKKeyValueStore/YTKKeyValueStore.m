...
- (id)getAllModelObjectByClassName:(Class)className fromTable:(NSString *)tableName
{
    NSArray * items = [self getAllItemsFromTable:tableName];
    NSMutableArray * mArr = [NSMutableArray array];
    
    for (int i = 0; i < items.count; i++) {
        YTKKeyValueItem * item = (YTKKeyValueItem *)items[i];
        [mArr addObject:item.itemObject];
    }
    
    return [className objectArrayWithKeyValuesArray:mArr];
}
